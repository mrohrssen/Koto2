...
## Sprite & Background Cache Busting

All `.webp` and `.mp3` files under `public/assets/` are served with 1-year immutable cache headers (see `server.js`). The browser pins each URL forever, so regenerated bytes won't reach players unless the URL changes. Two version constants gate this:

- **Sprites:** bump `SPRITE_VERSION` in `public/js/ui/sprite-utils.js` after regenerating any creature sprite.
- **Backgrounds:** bump `BACKGROUND_VERSION` in `public/js/pixi/parallax.js` after regenerating any area `sky.webp` / `battleground.webp` (or any other parallax layer).

Use the date of generation, e.g. `'20260505'`. Forgetting to bump means every player keeps seeing whatever older bytes their browser cached.
...
- **Don't forget to bump `SPRITE_VERSION` or `BACKGROUND_VERSION`** - After regenerating sprites bump `SPRITE_VERSION` in `public/js/ui/sprite-utils.js`; after regenerating area backgrounds bump `BACKGROUND_VERSION` in `public/js/pixi/parallax.js`. Without the bump, browsers serve the old cached bytes forever (1-year immutable cache).
