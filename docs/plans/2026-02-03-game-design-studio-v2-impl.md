# Game Design Studio v2 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite `/game-design-studio` skill to use micro-agent chains with file-based handoffs, preventing context limit issues.

**Architecture:** Each of 10 specialists becomes a 3-step chain (Researcher → Analyzer → Writer). Agents write to files, return one-line status. Orchestrator stays lean. Debate clusters and synthesis read from files.

**Tech Stack:** Claude Code skill (markdown), Task tool with `general-purpose` subagent type, Write/Read tools for file handoffs.

---

## Task 1: Create Directory Structure and Status Tracking Helpers

**Files:**
- Create: `docs/design-studio/tmp/.gitkeep` (placeholder for tmp dir)
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Create tmp directory placeholder**

Create `docs/design-studio/tmp/.gitkeep`:
```
# Temporary files for game design studio runs
# This directory is cleared after successful runs
```

**Step 2: Add file header and constants to skill**

Replace the entire content of `.claude/commands/game-design-studio.md` with the new v2 header section. We'll build incrementally.

Replace lines 1-9 of `.claude/commands/game-design-studio.md` with:
...
**Step 3: Verify skill file renders**

Run: `head -30 .claude/commands/game-design-studio.md`
Expected: Shows the new header with constants section

**Step 4: Commit**

```bash
mkdir -p docs/design-studio/tmp
echo "# Temporary files for game design studio runs" > docs/design-studio/tmp/.gitkeep
git add docs/design-studio/tmp/.gitkeep .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add v2 header and tmp directory structure"
```

---

## Task 2: Implement Request Parsing Section

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add Step 1 - Parse Request section**
...
Run: `grep -n "Parse the Request" .claude/commands/game-design-studio.md`
Expected: Shows line number with "Parse the Request"

**Step 3: Commit**

```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add request parsing and timing sections"
```

---

## Task 3: Implement Phase 1 - Codebase Analysis
...
After Step 3 (Create Tmp Directory), add:
...
Run: `grep -n "Phase 1 - Codebase" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header

**Step 3: Commit**

```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add phase 1 codebase analysis"
```

---

## Task 4: Implement Micro-Agent Prompt Templates

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add Researcher template**

After Phase 1 section, add:
...
**Step 4: Verify templates added**

Run: `grep -c "Template" .claude/commands/game-design-studio.md`
Expected: Shows count of 3 or more

**Step 5: Commit**

```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add micro-agent prompt templates"
```

---

## Task 5: Implement Specialist Definitions

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add specialist definitions section**

After the Writer Template, add:
...
Run: `grep -n "Specialist Definitions" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header
...
```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add specialist definitions table"
```

---

## Task 6: Implement Phase 2 - Micro-Agent Chain Spawning

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add Phase 2 orchestration logic**

After Specialist Definitions, add:
...
**Step 2: Verify phase 2 section added**

Run: `grep -n "Phase 2 - Parallel" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header
...
```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add phase 2 chain spawning logic"
```

---

## Task 7: Implement Error Handling and Retry Logic
...
Run: `grep -n "Failure Protocol" .claude/commands/game-design-studio.md`
Expected: Shows line number with section

**Step 3: Commit**

```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add error handling and retry logic"
```

---

## Task 8: Implement Phase 3 - Debate Clusters

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add Debate Cluster Template**

After Error Handling section, add:
...
**Step 2: Add Phase 3 orchestration**
...
**Step 3: Verify phase 3 added**

Run: `grep -n "Phase 3 - Clustered" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header
...
```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add phase 3 debate clusters"
```

---

## Task 9: Implement Phase 4 - Synthesis

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add synthesis section**

After Phase 3 section, add:
...
**Step 2: Verify phase 4 added**

Run: `grep -n "Phase 4 - Synthesis" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header
...
```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add phase 4 synthesis"
```

---

## Task 10: Implement Cleanup and Completion

**Files:**
- Modify: `.claude/commands/game-design-studio.md`

**Step 1: Add cleanup and completion section**

After Phase 4 section, add:
...
**Step 2: Verify cleanup section added**

Run: `grep -n "Cleanup and Completion" .claude/commands/game-design-studio.md`
Expected: Shows line number with section header
...
```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): add cleanup and completion handling"
```

---

## Task 11: Add Important Notes and Final Review
...
Run: `grep -E "^## " .claude/commands/game-design-studio.md | head -20`
Expected: Shows all major sections in order

**Step 3: Final commit**

```bash
git add .claude/commands/game-design-studio.md
git commit -m "feat(design-studio): complete v2 implementation with micro-agent chains"
```
...
Invoke the skill with a test request:
```
/game-design-studio 30 min focus on chip system, compare to Balatro
```
...
══════════════════════════════════════════════════════════════════
  ...
  Focus area:       chip system
  Comparison games: Balatro
══════════════════════════════════════════════════════════════════
```

**Step 3: Monitor first few agents**

Watch for:
- 9 Researcher agents launching in parallel
- Status returns like "Combat-Researcher: done, 3 sources"
- Files appearing in `docs/design-studio/tmp/`

**Step 4: Abort after confirming pattern**

If the parallel launch and file handoff pattern works correctly, abort the run (it will take 30+ minutes to complete fully).
...
| Task | What It Adds |
|------|--------------|
| 1 | Directory structure, header, constants |
| 2 | Request parsing, timing calculation |
| 3 | Phase 1 codebase analysis |
| 4 | Micro-agent prompt templates (Researcher, Analyzer, Writer) |
| 5 | Specialist definitions table |
| 6 | Phase 2 chain spawning orchestration |
| 7 | Error handling and retry logic |
| 8 | Phase 3 debate clusters |
| 9 | Phase 4 synthesis |
| 10 | Cleanup and completion |
| 11 | Important notes and final commit |
| 12 | Integration test |

Total: 12 tasks, ~11 commits
