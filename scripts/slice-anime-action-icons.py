#!/usr/bin/env python3
"""
Slice anime-style action icon grids into individual PNGs.

Reads anime-batch-*.png grids + their manifests from data/action-icon-staging-anime/,
slices into 128x128 icons, outputs to data/action-icon-staging-anime/sliced/.

Usage:
    python3 scripts/slice-anime-action-icons.py              # Process all grids
    python3 scripts/slice-anime-action-icons.py --batch 0    # Process specific batch
"""
...
STAGING_DIR = PROJECT_ROOT / "data" / "action-icon-staging-anime"
...
def process_batch(batch_idx, icon_size):
    grid_path = STAGING_DIR / f"anime-batch-{batch_idx}.png"
    manifest_path = STAGING_DIR / f"anime-batch-{batch_idx}-manifest.json"
    ...
    print(f"  {sliced} icons sliced")
    return True


def main():
    parser = argparse.ArgumentParser(description="Slice anime action icon grids")
    parser.add_argument("--batch", type=int, default=None)
    parser.add_argument("--size", type=int, default=128)
    args = parser.parse_args()

    if args.batch is not None:
        print(f"=== Anime Batch {args.batch} ===")
        process_batch(args.batch, args.size)
    else:
        grid_files = sorted(STAGING_DIR.glob("anime-batch-*.png"))
        if not grid_files:
            print("No anime grid files found in", STAGING_DIR)
            return
        for gf in grid_files:
            idx = int(gf.stem.replace("anime-batch-", ""))
            print(f"=== Anime Batch {idx} ===")
