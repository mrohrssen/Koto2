#!/usr/bin/env python3
"""
Remove backgrounds from sliced action icons using ComfyUI RMBG-2.0.

Uploads each 128×128 icon, runs RMBG, downloads transparent result,
and saves as PNG + WebP to public/assets/sprites/actions/.

Usage:
    python3 scripts/rembg-action-icons.py
    python3 scripts/rembg-action-icons.py --ids blanket,chomp,cut
"""
...
SLICED_DIR = PROJECT_ROOT / "data" / "action-icon-staging" / "sliced"
OUTPUT_DIR = PROJECT_ROOT / "public" / "assets" / "sprites" / "actions"
...
def build_rmbg_workflow(server_filename, icon_id):
    return {
        "prompt": {
            "1": {
            ...
            "3": {
                ...
                "inputs": {
                    "images": ["2", 0],
                    "filename_prefix": f"rembg_actions/{icon_id}",
                },
            },
        }
    }
...
def process_icon(icon_id):
    input_path = SLICED_DIR / f"{icon_id}.png"
    ...
    # Queue RMBG
    workflow = build_rmbg_workflow(server_name, icon_id)
    ...
    result_img.save(OUTPUT_DIR / f"{icon_id}.png", "PNG")
    result_img.save(OUTPUT_DIR / f"{icon_id}.webp", "WEBP", quality=90)
...
def main():
    ...
    if args.ids:
        icon_ids = args.ids.split(",")
    else:
        icon_ids = sorted([p.stem for p in SLICED_DIR.glob("*.png")])

    print(f"RMBG Action Icons — {len(icon_ids)} icons")
