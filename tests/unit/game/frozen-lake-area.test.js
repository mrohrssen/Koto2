...
const EXPECTED_SPAWNS = [
  'koori', 'kuma', 'tsuru', 'kamo', 'sakana', 'mizu'
];

describe('frozen-lake area', () => {
  const area = areas.find(a => a.id === 'frozen-lake');

  it('exists at array index 9 (after thunder-mountain)', () => {
    assert.ok(area, 'frozen-lake missing from areas.json');
    assert.equal(areas.indexOf(area), 9);
  });

  it('has the approved names and reading', () => {
    assert.equal(area.name, '氷の湖');
    assert.equal(area.nameEn, 'Frozen Lake');
    assert.equal(area.reading, 'こおりのみずうみ');
  });

  it('uses the object-form modifier/location schema', () => {
    assert.equal(area.modifierWord.word, '氷');
    assert.equal(area.locationWord.word, '湖');
  });

  it('has the approved spawn pool and boss', () => {
    assert.deepEqual(area.creatures, EXPECTED_SPAWNS);
    assert.equal(area.bossCreatureId, 'koorino-kuma');
  });
  ...
  it('has run wiring fields', () => {
    ...
    assert.equal(area.parallaxId, 'frozen-lake');
  });
});
