...
describe('Whack-a-Mole Room', () => {
  // Koto2 generateAreaRooms no longer produces whackAMole rooms (disabled room type),
  // but the room type constant and createRoom still work for direct creation.
  it('whackAMole is not generated by Koto2 30-room generator', () => {
    const rooms = generateAreaRooms('okunomori');
    const hasWAM = rooms.some(r => r.type === 'whackAMole');
    assert.strictEqual(hasWAM, false, 'whackAMole should not appear in Koto2 room generation');
  });
