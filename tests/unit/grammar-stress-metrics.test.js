...
describe('grammar stress metrics', () => {
  const catalog = loadGrammarCatalog();
  const matchers = loadGrammarMatchers();

  for (const { level, stressPath } of LEVELS) {
    describe(`${level} grammar stress metrics`, () => {
      const enabledIds = new Set(catalog.filter(p => p.level === level && p.status === 'enabled').map(p => p.id));
      const stressCases = JSON.parse(readFileSync(stressPath, 'utf-8'));
      ...
      it('only expects enabled grammar IDs for this level', () => {
        const levelPrefix = `${level.toLowerCase()}-`;
        const failures = [];
        for (const testCase of stressCases) {
          for (const id of testCase.expected || []) {
            if (id.startsWith(levelPrefix) && !enabledIds.has(id)) {
              failures.push(`${testCase.sentence}: ${id}`);
            }
          }
        }
        assert.deepEqual(failures, []);
